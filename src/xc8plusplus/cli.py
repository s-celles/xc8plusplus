...
def transpile(
    ...
    try:
        ...
        if success:
            console.print(f"[bold green]✅ Success![/bold green] Transpiled {input_file} → {output_file}")
            
            if verbose:
                console.print(f"\n[bold]Analysis Results:[/bold]")
                console.print(f"  Classes found: {len(transpiler.classes)}")
                for class_name, info in transpiler.classes.items():
                    console.print(f"    • {class_name}: {len(info['methods'])} methods, {len(info['fields'])} fields")
        else:
...
def demo() -> None:
    ...
    console.print("  • [green]Semantic transformation[/green] - Understands C++ constructs")
    console.print("  • [green]XC8 compatibility[/green] - Generates optimized C code")
    console.print("  • [green]Memory efficient[/green] - Suitable for 8-bit microcontrollers")
